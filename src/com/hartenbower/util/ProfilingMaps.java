...
public class ProfilingMaps {
          ...
          try {
            if (deltaMap != null && deltaMap.keySet() != null) {
              ...
              for (String sig : deltaMap.keySet()) {
                ...
                if (sig != null && deltas != null && !deltas.isEmpty()) {
                  for (Long called : deltas) {
                    if (!sig.startsWith(EpicMethod.class.getName())
                        && called != null && now - called > (Util.SIGMA * 1000)) {
                      foundBlocked = true;
                      sb.append(sig);
                      sb.append(": ");
                      sb.append(TimingUtil.fromSeconds((now - called) / 1000.));
                      sb.append("\n");
                    }
                  }
                }
  ...
  static class MethodExecutionProfile implements
    ...
    public MethodExecutionProfile(String signature, Double time, Long counter) {
    ...
    final static String one = StringUtil.hollowTo(1 + "", 10);
    final static String tmNA = StringUtil.hollowTo("NA", 20);
    final static String rateNA = StringUtil.hollowTo("NA", 30);

    @Override
    public String toString() {
      // 60 for method
      String meth = StringUtil.hollowTo(signature, 70);
      String countr = StringUtil.hollowTo(counter + "", 10);
      String tm = StringUtil.hollowTo(TimingUtil.fromSeconds(time), 20);
      String initCnt = StringUtil.hollowTo(initializerCount + "", 20);

      if (counter > 1) {
        String rate = StringUtil.hollowTo(decimal3.format(time / counter), 30);
        return meth + countr + tm + rate;
      } else if (initializerCount > 1) {
        return meth + initCnt + tmNA + rateNA;
      } else {
        return meth + one + tm + rateNA;
      }
    }
  ...
  public static String getTopXHtml(int maxCount) {
    ...
    StringBuilder sb = new StringBuilder("<h1>Top " + maxCount
        + " time-consuming invocations</h1>");
  ...
  public static String getTopX(int maxCount) {
    ...
    for (String methodShortSig : new ArrayList<String>(aggregateTimes.keySet())) {
      ...
      topScores.add(new MethodExecutionProfile(methodShortSig, test, count));
    }
    StringBuffer sb = new StringBuffer();
    // sb.append("Top time-consuming methods:\n\n");
    // sb.append("map sizes: aggTimes " + aggregateTimes.size() + ", counters "
    // + counters.size() + "\n\n");
    // sb.append("caching:  hits " + CacheUtil.hits + "; misses " +
    // CacheUtil.misses);
    int howMany = 0;
  ...
  public static String getTopXInits(int maxCount) {
    ...
    sb.append("@Cache and CacheUtil.cache(..) hits: " + CacheUtil.hits
        + "; misses " + CacheUtil.misses + " ratio "
        + NumberFormats.decimal3(1. * CacheUtil.hits / CacheUtil.misses)
        + "\n\n");
